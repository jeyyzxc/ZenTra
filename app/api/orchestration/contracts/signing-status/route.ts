...
export async function POST(request: Request) {
  try {
    ...
    return dashboardCreated({ id: workflowLog.id, contractId: contract.id }, 'Contract signing status saved successfully.');
  } catch (error) {
    return dashboardError(error, 'Unable to save contract signing status.');
  }
}
